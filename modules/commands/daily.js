let works = {
    '😆': {
        ...
        img: 'https://i.imgur.com/DoB5Cw8.gif',
              ...
              ['{name} bạn vừa bắt được cá vượng miệng rộng và bán được {money}$', 'https://i.imgur.com/Cw0qh57.png'],
    ...
    '😮': {
        name: 'đi săn',
        img: 'https://i.imgur.com/NQNURMP.gif',
        done: [
...
exports.handleReaction = async o=>{
    ...
    if (typeof data.work != undefined && data.work>= Date.now())return (x=>send(`⚠️ Bạn đã làm việc rồi\nHãy quay lại sau: ${_0(x/1000/60/60<<0)} giờ ${_0(x/1000/60%60<<0)} phút ${_0(x/1000%60<<0)} giây.`))(data.work-Date.now());

    let work = works[o.event.reaction];
    let msg = {};

    if (!work)return send(`❎ Công việc không có trong danh sách`);
  o.api.unsendMessage(o.handleReaction.messageID)
    data.work = Date.now()+(1000*60*60*3);
